  ...
  return withDb(async () => {
    try {
      ...
      try {
        ...
        console.error("Failed to load student profiles for progress:", e);
      }
      ...
      try {
        ...
        console.warn("Failed to load track counts:", e);
      }
          ...
          <div className="space-y-8">
            {profiles && profiles.length > 0 ? (
              profiles.map((profile) => {
                ...
                const completedCount = profile.progressRecords?.length ?? 0;
                const progress = Math.min(100, Math.round((completedCount / totalInTrack) * 100));
                const latestRecord = profile.progressRecords?.[0];
                const trackName = profile.track
                  ? String(profile.track).charAt(0).toUpperCase() + String(profile.track).slice(1).toLowerCase().replace(/_/g, " ")
                  : "Qaidah";
                  ...
                  <div key={profile.id} className="card">
                    ...
                    <div className="p-5" style={{ borderBottom: "1px solid var(--border)" }}>
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium" style={{ color: "var(--text-secondary)" }}>
                          {latestRecord ? `Last: ${latestRecord.lesson?.title || "Lesson"}` : "Just started..."}
                        </span>
                    ...
                    <div className="p-5">
                      ...
                      <div className="space-y-4">
                        {profile.progressRecords && profile.progressRecords.length > 0 ? (
                          profile.progressRecords.slice(0, 5).map((record: any) => (
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center justify-between">
                                  <span className="text-sm font-medium" style={{ color: "var(--text-primary)" }}>
                                    {record.lesson?.title || "Completed Lesson"}
                                  </span>
                                  <span className="text-xs" style={{ color: "var(--text-tertiary)" }}>
                                    {record.completedAt ? format(new Date(record.completedAt), "MMM d") : "Completed"}
                                  </span>
                                </div>
                                {record.teacherNotes && (
              ...
              <div className="card p-10 text-center space-y-4">
                ...
                <Link
                  ...
                  className="inline-block px-5 py-2.5 rounded-xl text-xs font-semibold bg-emerald-600 text-white hover:bg-emerald-500 transition shadow-xs"
                >
                  Book a Trial Class
                </Link>
              </div>
      ...
      console.error("ProgressPage top-level error:", err);
      return (
        <div className="p-6 lg:p-10 max-w-5xl">
          <div className="card p-10 text-center space-y-3">
            <h2 className="text-lg font-bold" style={{ color: "var(--text-primary)" }}>Student Progress</h2>
            <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
              Unable to load progress at this moment.
            </p>
            <Link
              href="/dashboard"
              className="inline-block px-4 py-2 rounded-xl text-xs font-semibold bg-slate-800 text-white hover:bg-slate-700 transition"
            >
              Back to Dashboard
            </Link>
          </div>
