...
export async function copyTextToClipboard(text: string): Promise<boolean> {
  ...
  // 2. Fallback: Android & iOS compatible in-DOM selection + execCommand('copy')
  if (typeof document !== 'undefined') {
    try {
      const textarea = document.createElement('textarea');
      textarea.value = text;
      // Note: Do NOT add 'readonly' attribute — on Android Chrome/WebKit readonly blocks text selection!
      ...
      textarea.style.opacity = '0.01'; // Non-zero opacity so mobile browsers do not cull selection
